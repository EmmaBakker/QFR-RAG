...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Unified fusion evaluation CLI (linear + RRF) for QFR-RAG. Select task with --dataset.",
...
""",
    ...
    ap.add_argument(
        ...
        required=True,
        choices=["A", "B"],
        help="QFR task to evaluate: A (Task A chunk-level) or B (Task B slot-based).",
    )
    ...
    ap.add_argument("--agg", choices=["max", "sum"], default="max",
                    help="Aggregate fused chunk scores to docs (default: max).")
