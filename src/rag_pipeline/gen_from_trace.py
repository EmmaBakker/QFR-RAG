...
# NOTE: No implicit NIC normalization is applied in this script.
# The prompt instructs the model to output exactly NIC_TOKEN when evidence is
# absent.  Semantic refusal phrases are not mapped to the NIC token; with
# structured JSON output they should not appear in the answer field, and
# silently normalizing them would obscure generation failures.
# The only post-generation intervention is forced NIC (see below).
...
def _task_name_for_prompt(task: str) -> str:
    task = task.upper()
    if task == "A":
        return "taskA"
    if task == "B":
        return "taskB"
    raise ValueError(f"Unsupported task: {task!r}. Expected 'A' or 'B'.")
